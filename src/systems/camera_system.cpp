...
void zoom(const components::Mouse &m1, const components::Update<components::Mouse> &u, const components::ViewPort &v,
          components::CameraPersp &c)
{
    ...
    const auto newPivotDistance =
      powf(2.71828183f, 2 * -mouseDelta / length(glm::vec2(v.value.z, v.value.w))) * initialPivotDistance;
...
void pan(const components::Mouse &m1, const components::Update<components::Mouse> &u, const components::ViewPort &v,
         components::CameraPersp &c)
{
    const auto &m2 = u.component;
    //    auto &c = uC.component;
    ...
    if (const auto eyePoint = c.getEyePoint() - right * mouseDelta.x + up * mouseDelta.y;
        !glm::any(glm::isnan(eyePoint)))
    {
        c.setEyePoint(eyePoint);
    }
}

void orbit(const components::Mouse &m1, const components::Update<components::Mouse> &u, const components::ViewPort &v,
           components::CameraPersp &c)
{
    const auto &m2 = u.component;
    //    auto &c = uC.component;
    ...
    if (!glm::any(glm::isnan(rotatedVec)))
    {
        ...
        c.setOrientation(glm::angleAxis(mouseDelta.x, c.getWorldUp()) * glm::angleAxis(mouseDelta.y, mU) *
                         c.getOrientation());
    }
}
...
{
    r.view<components::Mouse, components::Update<components::Mouse>, components::ViewPort, components::CameraPersp>()
      .each([](auto e, const auto &m, const auto &uM, const auto &v, auto &c) {
          ...
          const auto leftDrag = m.buttons[components::Mouse::MouseButton::Left] &&
                                uM.component.buttons[components::Mouse::MouseButton::Left];
          const auto rightDrag = m.buttons[components::Mouse::MouseButton::Right] &&
                                 uM.component.buttons[components::Mouse::MouseButton::Right];
