...
glm::vec2 Camera::worldToScreen( const glm::vec3 &worldCoord, float screenWidth, float screenHeight ) const
{
	...
	return glm::vec2( ( ndc.x + 1.0f ) / 2.0f * screenWidth, ( 1.0f - ( ndc.y + 1.0f ) / 2.0f ) * screenHeight );
}
...
glm::vec2 Camera::eyeToScreen( const glm::vec3 &eyeCoord, const glm::vec2 &screenSizePixels ) const
{
	...
	return glm::vec2( ( ndc.x + 1.0f ) / 2.0f * screenSizePixels.x, ( 1.0f - ( ndc.y + 1.0f ) / 2.0f ) * screenSizePixels.y );
}
...
glm::vec3 Camera::worldToNdc( const glm::vec3 &worldCoord ) const
{
	...
	return glm::vec3( unproj.x / unproj.w, unproj.y / unproj.w, unproj.z / unproj.w );
}
...
void Camera::getClipCoordinates( float clipDist, float ratio, glm::vec3* topLeft, glm::vec3* topRight, glm::vec3* bottomLeft, glm::vec3* bottomRight ) const
{
	...
	const glm::vec3 viewDirection = normalize( mViewDirection );
...
CameraPersp::CameraPersp( int pixelWidth, int pixelHeight, float fovDegrees, float nearPlane, float farPlane )
{
	...
	theTan 			= tan( halfFov );
	float dist 		= eyeY / theTan;
	aspect 			= pixelWidth / static_cast<float>(pixelHeight);
